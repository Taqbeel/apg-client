...
import { getAlbOrders, assignOrder, uploadManualOrders } from '@/api/orders';
...
const Orders: React.FC = () => {
  ...
  const columns: TableProps<DataType>['columns'] = [
    ...
    },
    {
      ...
      render: (data) => {
        return(
          <>
            <div className='text-[10px] font-bold'>{dayjs(data.PurchaseDate).fromNow()}</div>
            <div className='text-[10px] font-semibold text-gray-400'>{dayjs(data.PurchaseDate).format("D/M/YYYY")}</div>
            <div className='text-[10px] font-semibold text-gray-400'>{dayjs(data.PurchaseDate).format("hh:mm a")}</div>
          </>
        )
      },
    },
    {
      ...
      render: (data) => {
        return(
          <>
            <Link href={`/dashboard/alb-order?id=${data['AmazonOrderId']}`} className='text-[11px] font-bold'>
              {data['AmazonOrderId']}
            </Link>
            <div className='text-[10px] font-semibold text-gray-400'>Buyer Name</div>
            <div className='text-[10px] font-semibold text-gray-400'>{data['BuyerInfo']?.Name||''}</div>
            <div className='text-[10px] font-semibold text-gray-400'>Sales Channel: {data['SalesChannel']}</div>
          </>
        )
      },
    },
            ...
            <div key={item.id}>
              <div className='flex mt-2'>
                ...
                <div className='mx-3'>
                  ...
                  <div className='text-[10px] text-gray-500 mt-1' style={{lineHeight:1.1}}>
                    <a href={`https://www.amazon.com/gp/product/${item.ASIN}`} target='_blank'>
                      ASIN:
                  ...
                  <div className='text-[10px] text-gray-500 my-1' style={{lineHeight:1.1}}>
                    SKU:
                    <span className='font-semibold'>
                      {" "}{item.SellerSKU}
                    </span>
                  </div>
                  <div className='text-[10px] text-gray-500' style={{lineHeight:1.1}}>
                    Quantity:
                    <span className='font-semibold'>
      ...
      render: (data) => {
        return(
          <>
            ...
            <div className='text-[10px] font-semibold text-gray-400'>
              Delivery: {dayjs(data.LatestDeliveryDate).format("dd, MMM D, hh:mm a")}
            </div>
            }
          </>
        )
      ...
      render: (data) => {
        return(
          <b className='blue-2-txt'>
            {status=="arriving" &&
              <Tag color={data.status=="Pending"?"#f50":data.status=="Unshipped"?"#2db7f5":"green"}>
                <div className='text-[10px]'>{data.status}</div>
              </Tag>
            }
            {status!="arriving" &&
              <Tag color={data.OrderStatus=="Pending"?"#f50":data.OrderStatus=="Unshipped"?"#2db7f5":"green"}>
    ...
    },
    {
      ...
      render: (data) => {
        return(
          <div className='flex'>
            {data.User && <span className='text-green-400'><FaUserCheck /></span>}
            <span className='mx-1'></span>
            {data.User}
            {!data.User && <span className='text-gray-400'><FaUserSlash /></span>}
          </div>
        )
      },
    },
            ...
            {status!="arriving" && <>
              {(status!="transit" && status!="delivered") && <>
                <div className='shipment-btn text-[11px]'>
                  <Link href={`/dashboard/shipment?id=${data['AmazonOrderId']}`}>
                    <span className='no-underline text-white'>Buy Shipping</span>
                  </Link>
                </div>
              </>
              }
              ...
              <>
                <div className='shipment-btn text-[11px] my-2'>
                  <Link href={`/dashboard/tracking?id=${data['AmazonOrderId']}`}>
                    <span className='no-underline text-white'>Order Tracking</span>
                  </Link>
                </div>
                <div className='text-[11px]'>
                  <div>Expected Delivery By:</div>
                  <div>
                    <b>{dayjs(data['OrderShipment']?.promise?.deliveryWindow.end).format("ddd DD, MMMM - hh:mm a")}</b>
                  </div>
                </div>
              </>
              }
    ...
    data.forEach((x:any) => {
      if(orders.length==0){
        ...
        if(orders[orders.length-1].AmazonOrderId==x['order-id']){
          ...
          orders[orders.length-1].Amount = orders[orders.length-1].Amount + Number(x["item-price"]) + Number(x["item-tax"])
        } else {
          orders.push(orderObject(x))
        }
      }
  ...
  const TableComponent = () =>  
    <div className='my-4' style={{maxHeight:500, overflowY:'auto'}}>
      <Table columns={columns} dataSource={data?.filter((x:any)=>x.status==status)} />
    </div>
        ...
        <div>
          {checkedOrders.length>0 && 
            <Select
              ...
              options={userList.map((x:any)=>{ return { value:x.id, label:x.name } })}
            />
          }
        </div>
        <div>
