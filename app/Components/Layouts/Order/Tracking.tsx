...
import { Col, Flex, Row, Spin, Timeline } from "antd";
...
const Tracking = () => {
  ...
  useEffect(() => {
    ...
    const vendorName: any = searchParams.get("vendorName");
    console.log("id", id);
    console.log("vendorName", vendorName);
    fetchOrderData(id, vendorName);
  }, []);

  const fetchOrderData = (id: string, vendorName: string) => {
    getTracking({
      id,
      vendorName,
    }).then((x) => {
              ...
              <div className="tracking-info-box">
                ...
                <div className="my-3">
                  <b className="text-[20px] text-blue-900">
                    {dayjs(order.promisedDeliveryDate)
                      .format("dddd")
                      ?.toUpperCase()}
                  </b>
                  <br />
                    ...
                    <div>
                      <Flex>
                        <div className="text-[12px] text-blue-900 pt-6">
                          <b>
                            {dayjs(order.promisedDeliveryDate).format("MMMM")}
                          </b>
                          <br />
                          <b>
                            {dayjs(order.promisedDeliveryDate).format("YYYY")}
                          </b>
                        </div>
                        <div
                          className="pt-3 px-3 mx-3"
                          style={{ borderLeft: "1px solid grey" }}
                        >
                          <div>BY</div>
                          <b className="text-[20px] text-blue-900">
                            {dayjs(order.promisedDeliveryDate).format(
                              "hh:mm a"
                            )}
                          </b>
                        </div>
              ...
              >
                {order.eventHistory && (
                  <Timeline
                    pending="Delivering..."
                    reverse={true}
                    items={order?.eventHistory
                      .filter((x: any) => {
                        return (
                          x.eventCode == "ReadyForReceive" ||
                      ...
                      .map((x: any) => {
                        return {
                          children: (
                            <>
                              <div>
                                <b className="text-[12px] text-blue-900">
                                  {codeMaking(x.eventCode)}
                                </b>
                                <div>
                                  {x?.location?.city?.toUpperCase()}{" "}
                                  {x?.location?.city ? ", " : ""}
                                  {x?.location?.stateOrRegion?.toUpperCase()}{" "}
                                  {x?.location?.stateOrRegion ? ", " : ""}
                                  {x?.location?.countryCode?.toUpperCase()}
                                </div>
                                <div className="text-[11px]">
                                  {/* {dayjs.tz(x.eventTime, "America/Toronto").format("MMMM DD, YYYY, hh:mm ")} */}
                                  {dayjs(x.eventTime).format(
