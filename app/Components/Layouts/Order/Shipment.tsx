...
const Shipment = () => {
  ...
  const fetchRates = () => {
    ...
    let body = {
      shipTo: {
        ...
        addressLine1: order?.BuyerInfo?.AddressLine1,
        stateOrRegion: order?.BuyerInfo?.StateOrRegion,
        city: order?.BuyerInfo?.City,
        countryCode: order?.BuyerInfo?.CountryCode,
        postalCode: order?.BuyerInfo?.PostalCode,
        phoneNumber: order?.BuyerInfo?.Phone?.slice(3,15),
      },
      shipFrom: {
        name: "Shipping Guru NJ",
        addressLine1: "86 Lackawanna avenue",
      ...
      packages:[
        {
          ...
          isHazmat: false,
          sellerDisplayName: order?.BuyerInfo?.Name,
          charges: [
            {
              amount: {
          ...
          packageClientReferenceId: order?.BuyerInfo?.Name||`AMAZON${order.AmazonOrderId}`,
          items:items
        }
      ],
      channelDetails: {
    ...
    try {
      getRates(body).then((x) => {
        // console.log('responce', JSON.parse(x.data.result))
        if(x.data.status=="success"){
          let temp = JSON.parse(x.data.result)?.payload?.rates.map((rate:any)=>{ return{...rate, isCheck:false}});
          setRates({...JSON.parse(x.data.result).payload, rates:temp.sort((a:any, b:any) => a.totalCharge.value - b.totalCharge.value)});
        }
        setPriceLoad(false)
      });
    } catch (error) {
  ...
  const confirmShipment = () => {
    ...
    let formatType:any = tempRate.supportedDocumentSpecifications.filter((x:any)=>x.format=="PDF"||x.format=="PNG")[0];
    const payload = {
      body:{
        ...
        requestedDocumentSpecification:{
          ...
          needFileJoining:formatType?.printOptions[0]?.supportedFileJoiningOptions[0],
          requestedDocumentTypes:formatType?.printOptions[0]?.supportedDocumentDetails.map((x:any)=>{return x.name}),
        },
        requestedValueAddedServices:tempRate.availableValueAddedServiceGroups.map((x:any)=>{ return{id:x.groupId} })
  ...
  const downloadLabel = () => {
    ...
    getLabel({
      ...
      if(x.data.status=="success"){
        ...
        setDocument(`data:${type}/${x.data.result.format};base64,${x.data.result.document}`)
        setIsModalOpen(true)
      }
    })
  }
            ...
            <Flex gap={'10px'}>
              ...
              <div>
                <div className='text-[12px] text-gray-500'>
                  ASIN: <span className='font-semibold'> {" "}{item.ASIN} </span>
                </div>
                <div className='text-[12px] text-gray-500'>
                  SKU: <span className='font-semibold'> {" "}{item.SellerSKU} </span>
                </div>
                <div className='text-[12px] text-gray-500'>
                  Order Item ID: <span className='font-semibold'> {" "}{item.OrderItemId} </span>
                </div>
                <div className='mt-1'>
    ...
    <Row>
      ...
      <Col span={6}>
        <Flex gap={5}>
          <div>
            Length:<b> {"(in)"}</b>
            <InputNumber value={dimension.length} onChange={(e)=>setDimension({...dimension, length:Number(e)})} min={0.1} />
          </div>
          <div>
            Height:<b> {"(in)"}</b>
            <InputNumber value={dimension.height} onChange={(e)=>setDimension({...dimension, height:Number(e)})} min={0.1} />
          </div>
          <div>
            Width:<b> {"(in)"}</b>
            <InputNumber value={dimension.width} onChange={(e)=>setDimension({...dimension, width:Number(e)})} min={0.1} />
          </div>
        </Flex>
      </Col>
      <Col span={2}></Col>
      <Col>
        <Flex gap={4}>
          <div>
            Weight <b> {"(lb)"}</b><br/>
            <InputNumber value={weight} onChange={(e)=>{ setOz(0); setWeight(Number(e)) }} />
          </div>
          <div>
            Weight <b> {"(oz)"}</b><br/>
            <InputNumber value={oz} onChange={(e)=>{ setWeight(0); setOz(Number(e))}} />
          </div>
        </Flex>
      </Col>
      ...
      <Col className='mx-4'>
        <br/>
        <button className='shipment-btn text-[14px]' onClick={fetchRates} disabled={priceLoad}>
          {!priceLoad?"Get Rates":<LoadingOutlined className='mx-5' />}
        </button>
      </Col>
      <Col className='mx-2'>
        <br/>
        ...
        <Row style={{maxHeight:'40vh', overflowY:'auto'}}>
          <Col span={23}>
            <div className='mt-1' >
              ...
              return(
                <div key={index} className={rate.isCheck?'checked-rate-row':'rate-row'}
                  onClick={()=>{
                    let temp:any = rates.rates.map((rate:any)=>{ return{...rate, isCheck:false}});
                ...
                >
                  ...
                  <div>
                    <p>
                      ...
                      <span className='text-[11px]' style={{position:'relative', bottom:5}}>
                        <span>{` ${rate.totalCharge.value}`.split(".")[1]}</span>
                      </span>
                    </p>
                    <p className='text-gray-400'>
                      {dayjs.tz(rate.promise.deliveryWindow.end, "America/Toronto").format("ddd, MMM DD")}
                    </p>
                  </div>
                </div>
              )})}
