    ...
    },
    {
      ...
      render: (data) => {
        ...
        <>
          ...
          <div>
            Item Total:{" "}
            <span className='font-semibold'>{parseFloat(data?.OrderTotal?.Amount)}</span>
          </div>
        </>
        )
      },
        ...
        <Row className='my-2'>
            ...
            <div className='text-[11px]  font-bold'>
              <Flex justify='space-between' gap={'20px'}>
                ...
                <Flex gap={'10px'}>
                  ...
                  <div className='w-3/4 text-yellow-1'>{dayjs(order.LatestShipDate).format("ddd, MMM D, YYYY")}</div>
                </Flex>
                </div>
                <div className='w-2/4'>
                  <Flex gap={'10px'}>
                ...
                <Flex gap={'10px'}>
                  ...
                  <div className='w-3/4 text-gray-400'>
                    {order.OrderStatus!="Pending" && <>
                      { dayjs(order.EarliestDeliveryDate).format("ddd, MMM D, YYYY")}
                      {" "}to{" "}
                      { dayjs(order.LatestDeliveryDate).format("ddd, MMM D, YYYY")}
                    </>
                    }
                    {order.OrderStatus=="Pending" && <div>Info Pending</div>}
                  </div>
                </Flex>
                </div>
                <div className='w-2/4'>
              ...
              <Flex justify='space-between' gap={'20px'} className='mt-2'>
                ...
                <Flex gap={'10px'}>
                  ...
                  <div className='w-3/4 text-gray-400'>
                    {dayjs(order.PurchaseDate).format("ddd, MMM D, YYYY, hh:mm a")}
                  </div>
                </Flex>
                </div>
                <div className='w-2/4'>
          ...
          <Col span={12} className='px-1'>
            <div className={`${borderClass} h-[150px]`}>
              ...
              {order.OrderStatus!="Pending" &&
                ...
                <Flex justify='space-between' gap={'20px'}>
                  <div>
                    ...
                    <div className='mt-1'>
                      Address Type:<br/>
                      <span className='text-gray-400'>{order?.BuyerInfo?.AddressType}</span>
                    </div>
                  </div>
                  <div>
                    Buyer Contact<br/>
                    <span className='text-gray-400'>
                      <Flex align='center'>
                        <FaPhone />
                        <span className='mx-2'>{order?.BuyerInfo?.Phone}</span>
                      </Flex>
                    </span>
                  </div>
                </Flex>
              ...
              {order.OrderStatus=="Pending" && <div className='text-[11px] text-gray-400 font-bold'>Shipper Info Pending</div>}
            </div>
          </Col>
          <Col span={24} className={`px-1 mt-2`}>
            <div className={borderClass}>
              ...
              <hr className='my-1' />
              <div>Order Total Amount: <span className='font-semibold'>{order?.OrderTotal?.Amount}</span></div>
            </div>
          </Col>
        </Row>
        <div className='my-4' style={{maxHeight:500, overflowY:'auto'}}>
          {order.OrderItems.length>0 && <Table columns={columns} dataSource={[order]} pagination={false} />}
        </div>
      </>
      }
      {load && 
